 ...
 *   ./macfan_smc set-rpm 3000 - set both fans to 3000 RPM (forced mode)
 *   ./macfan_smc set-auto     - return both fans to Apple auto control
 */
...
/* MACFAN_EMBEDDED tells smc.c to skip its own main() */
 ...
 * Clamps to each fan's min/max read from the SMC.
 * ------------------------------------------------------------------------- */
 ...
 * Always call this to hand control back to the SMC.
 * ------------------------------------------------------------------------- */
